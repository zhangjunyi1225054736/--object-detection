...
class JsonDataset(object):
    ...
    def __init__(self, name, test_id=-1):
        ...
        self.name = name
        self.imageset_dir = name[9:]
        self.image_directory = DATASETS[name][IM_DIR]
        self.image_prefix = (
            '' if IM_PREFIX not in DATASETS[name] else DATASETS[name][IM_PREFIX]
        )
        file_name = DATASETS[name][ANN_FN]
        if test_id>=0:
            tmp = DATASETS[name][ANN_FN].split('/')[-1].split('.')[0]+'{:d}.json'.format(test_id)
            file_name = '/'.join(DATASETS[name][ANN_FN].split('/')[:-1])+'/test/'+tmp
            #print(file_name)
            self.imageset_dir = 'test/'+'test{:d}'.format(test_id)
